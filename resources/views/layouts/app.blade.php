...
<body class="font-body min-h-screen flex flex-col antialiased @yield('body_class', 'bg-bg-light text-text-dark')">
    ...
    {{-- TODO: reemplazar 521XXXXXXXXXX con número real de WhatsApp del cliente --}}
    <a href="https://wa.me/521XXXXXXXXXX?text=Hola,%20me%20interesa%20saber%20m%C3%A1s%20sobre%20los%20lentes%20nuvion%20glass"
       target="_blank"
       rel="noopener"
       id="whatsapp-float"
       style="position:fixed;bottom:24px;right:24px;z-index:9999;width:52px;height:52px;border-radius:50%;background:#25D366;display:flex;align-items:center;justify-content:center;box-shadow:0 4px 16px rgba(37,211,102,0.4);transition:transform .2s ease,box-shadow .2s ease;text-decoration:none;"
    ...
    <div id="wa-tooltip" style="position:fixed;bottom:30px;right:86px;z-index:9998;background:#1a1a2e;color:#fff;font-size:12px;padding:6px 12px;border-radius:6px;white-space:nowrap;opacity:0;transition:opacity .2s ease;pointer-events:none;">¿Tienes dudas? Escríbenos</div>
    <script>
    (function(){
        var b=document.getElementById('whatsapp-float'),t=document.getElementById('wa-tooltip');
        if(b&&t){b.addEventListener('mouseenter',function(){t.style.opacity='1';});b.addEventListener('mouseleave',function(){t.style.opacity='0';});}
    })();
    </script>
</body>
</html>
