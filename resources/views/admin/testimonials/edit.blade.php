...
@section('content')
    <form method="POST" action="{{ route('admin.testimonials.update', $testimonial) }}" class="max-w-3xl space-y-6">
        @csrf @method('PUT')
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <textarea id="body" name="body" rows="3" required
                              class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('body', $testimonial->body) }}</textarea>
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <select id="rating" name="rating"
                                class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                            @for($i = 5; $i >= 1; $i--)
                            <option value="{{ $i }}" {{ old('rating', $testimonial->rating) == $i ? 'selected' : '' }}>{{ $i }} estrella{{ $i > 1 ? 's' : '' }}</option>
                            @endfor
                        </select>
                    </div>
                    <div>
                        <label for="avatar_color" class="block text-sm font-medium text-gray-700 mb-1">Color avatar</label>
                        <input type="color" id="avatar_color" name="avatar_color" value="{{ old('avatar_color', $testimonial->avatar_color) }}"
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" id="sort_order" name="sort_order" value="{{ old('sort_order', $testimonial->sort_order) }}" min="0"
                               class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                    <div>
                        ...
                        <label class="flex items-center gap-2 mt-2 cursor-pointer">
                            ...
                            <span class="text-sm text-gray-600">Activo (visible en el home)</span>
                        </label>
                    </div>
                </div>
            </div>
